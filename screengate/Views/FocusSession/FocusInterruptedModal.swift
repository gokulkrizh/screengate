...
struct FocusInterruptedModal: View {
    ...
    var isConfirmationValid: Bool {
        confirmationText.uppercased() == "I FORFEIT"
    }
        ...
        ZStack {
            ...
            VStack(spacing: 0) {
               // Spacer()
                ...
                // Modal content
                VStack(spacing: 20) {
                    // Warning header section with striped background
                    VStack(spacing: 20) {
                        // Warning header
                        VStack(spacing: 12) {
                            ZStack {
                    ...
                    HStack(spacing: 12) {
                        ...
                        Button(action: {
                            if isConfirmationValid && countdownTimer == 0 {
                                onEndSession()
                                isPresented = false
                            }
                        }) {
                            Text("End Session")
                                ...
                                .frame(maxWidth: .infinity)
                                .frame(height: 56)
                                .foregroundColor(.red.opacity(countdownTimer == 0 && isConfirmationValid ? 1 : 0.3))
                                .background(Color.red.opacity(0.08))
                                .border(Color.red.opacity(countdownTimer == 0 && isConfirmationValid ? 0.3 : 0.1), width: 1)
                                .cornerRadius(12)
                        }
                        .disabled(countdownTimer > 0 || !isConfirmationValid)
                    }
                    .padding(20)
                }
               // .padding(20)
